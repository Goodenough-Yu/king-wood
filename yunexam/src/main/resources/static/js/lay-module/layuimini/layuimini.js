...
layui.define(["element", "jquery", "layer"], function (exports) {
        ...
        layuimini = new function () {
            ...
            this.initPageTitle = function (href, title) {
                ...
                $("[data-one-template]").each(function () {
                    if ($(this).attr("data-one-template") == href) {
                        ...
                        $('.layuimini-template-header').removeClass('layui-hide');
                        $('#layuimini-template-header').empty().html(pageHeaderHtml);
                        return false;
                    }
                });
            };
            ...
            this.initConten = function (href, isHash) {
                var container = '.layuimini-content-template';
                ...
                $.each(data, function (key, val) {
                    ...
                    $.each(menuList, function (index, menu) {
                        ...
                        if (menu.child != undefined && menu.child != []) {
                            ...
                            var buildChildHtml = function (html, child, menuParameId) {
                                ...
                                $.each(child, function (childIndex, childMenu) {
                                    ...
                                    if (childMenu.child != undefined && childMenu.child != []) {
                                        ...
                                        html += '<a href="javascript:;" class="layui-menu-tips" data-type="tabAdd"  data-one-template-mpi="m-p-i-' + menuParameId + '" data-one-template="' + childMenu.href + '" target="' + childMenu.target + '"><i class="' + childMenu.icon + '"></i><span class="layui-left-nav"> ' + childMenu.title + '</span></a>\n';
                                        menuParameId++;
                            ...
                            leftMenuHtml += '<a href="javascript:;" class="layui-menu-tips"  data-type="tabAdd" data-one-template-mpi="m-p-i-' + menuParameId + '" data-one-template="' + menu.href + '" target="' + menu.target + '"><i class="' + menu.icon + '"></i><span class="layui-left-nav"> ' + menu.title + '</span></a>\n';
                            menuParameId++;
                        }
                        leftMenuHtml += '</li>\n';
                    });
         ...
         * 打开新窗口
         */
        $('body').on('click', '[data-one-template]', function () {
            var loading = layer.load(0, {shade: false, time: 2 * 1000});
            var href = $(this).attr('data-one-template'),
                title = $(this).text(),
